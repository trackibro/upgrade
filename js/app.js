...
document.addEventListener('click',e=>{if(e.target.classList.contains('tb')){
    const p=e.target.parentElement;
    p.querySelectorAll('.tb').forEach(t=>t.classList.remove('a'));e.target.classList.add('a');
    const pr=p.parentElement;
    pr.querySelectorAll('.tcc').forEach(t=>t.classList.remove('a'));
    const tg=pr.querySelector('#'+e.target.dataset.t);if(tg)tg.classList.add('a');
}});

function upD(){
    D.lastAct = Date.now(); sv(); 
